...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        rank_data = self.calc_result(person_datas, event_ids, competiton_datas, 'average')
    ...
    def calc_result(self, person_datas, event_ids, competiton_datas, rank_type):
        ...
        with open(self.get_fixtures_path('result')) as file:
            ...
            for event_id in event_ids:
                ...
                query = '[ [.[].fields ] | \
                    map(select ( .event_id == ' + str(event_id) + ' )) | \
                    group_by(.person_id) | .[] | sort_by(.' + rank_type + ') | \
                    .[0] ] | \
                    sort_by(.' + rank_type + ') | \
                    .[] | \
                    { id, event_id, person_id, ' + rank_type + ', competition_id, value1, value2, value3, value4, value5 }'
                ...
                for result in results:
                    if result[rank_type] > 0:
                        ...
                        if before_record == 0 or before_record < result[rank_type]:
                            ...
                            rank += skip_count
                            gender_ranks[person_datas[result['person_id']].gender] += skip_count
                            generation_ranks[generation] += skip_count
                            prefecture_ranks[person_datas[result['person_id']].prefecture_id] += skip_count
                            skip_count = 1
                        else:
                            skip_count += 1
                        ...
                        result['rank'] = rank
                        result['gender_rank'] = gender_ranks[person_datas[result['person_id']].gender]
                        result['generation_rank'] = generation_ranks[generation]
                        result['prefecture_rank'] = prefecture_ranks[person_datas[result['person_id']].prefecture_id]
