...
class Mine(LoginRequiredMixin, TemplateView):
    def get(self, request, **kwargs):
        ...
        # 参加した大会
        competitors = Competitor.objects.filter(person__id=person_id).exclude(
        ...
        for competition in competitions:
            ...
            # 非公開の企画している大会
            if competition.is_private and (
                request.user.is_superuser
                or person_id in competition.organizer_person_ids
                or person_id in competition.judge_person_ids
            ):
                private_competition_list.append(competition)

            # 非表示の企画している大会
            elif (
                not competition.is_display
                and not competition.is_private
                and (
                    request.user.is_superuser
                    or person_id in competition.organizer_person_ids
                    or person_id in competition.judge_person_ids
                ...
                not competition.is_private
                and competition.is_display
                and competition.id in participate_competition_ids
            ):
