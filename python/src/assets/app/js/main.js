$(function () {
  ...
  if (
    $(".registration_terms").prop("checked") &&
    $(".registration_privacy_policy").prop("checked")
  ) {
  ...
  $(".ranking_search").click(function () {
    var value = $(this).val();
    var input = $("<input>").attr("type", "hidden").attr("name", "type").val(value);
    $("form").append(input);
    $("form").submit();
  });
  ...
  // ボタン用Submit(action変更)
  $(".competition_admin_qrcode").click(function () {
    var value = $(this).val();
    $("form").attr("action", value);
